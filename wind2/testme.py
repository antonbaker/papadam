...
def allcalculations(
    wind_csv_df: DataFrame = None, rpm_csv_df: DataFrame = None
) -> tuple:
    """
    ...
    wind_csv_df["volt"] = wind_csv_df[
        "Differential pressure voltage as digital value in -"
    ]
    ...
    Rs = 287.058
    wind_csv_df["rho"] = (wind_csv_df["pabs"] * 100) / (
        Rs * (wind_csv_df["temp"] + 273.15)
    )
    ...
    fl = 25
    omega = signal.savgol_filter(
        rpm_csv_df["n"].astype(float) * 2 * np.pi / 60,
        window_length=fl,
        polyorder=3,
        deriv=0,
        delta=dt,
    ...
    v_fit = scipy.interpolate.interp1d(
        wind_csv_df["t_v"],
        wind_csv_df["v"],
        kind="slinear",
        bounds_error=False,
    ...
    p_fit = scipy.interpolate.interp1d(
        wind_csv_df["t_v"],
        wind_csv_df["pabs"],
        kind="slinear",
        bounds_error=False,
    ...
    temp_fit = scipy.interpolate.interp1d(
        wind_csv_df["t_v"],
        wind_csv_df["temp"],
        kind="slinear",
        bounds_error=False,
